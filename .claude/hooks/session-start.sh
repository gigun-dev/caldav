...
# 閾値: カタログ現況(2026-07-12 時点)は約 277 行 / 更新ブロック十数個。第2版の棚卸し直後の
# 分量に対し、次の棚卸しを促すべき「そろそろ溜まった」ラインとして下記を置く。ここを超えたら
# 第3版の棚卸し(積層を本文へ溶かし込み、頭を最新の現在地へ)を検討する合図。数字はあくまで
# 目安 — 棚卸ししたら現況に合わせて下げ直してよい(この値自体もメンテ対象)。
CATALOG_MAX_LINES=420
UPDATE_BLOCK_MAX=20
