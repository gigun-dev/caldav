...
## 2026-08-01: iOS 初回アカウント追加バグの切り分け + サーバー是正5件 + Apple クライアントモデル総当たり調査

- **発端**: まっさらな iOS Simulator に CalDAV アカウントを追加すると、サーバー検証(ホスト名/
  ユーザー名/パスワード確認画面)は通るのに、直後の「有効にするデータ」トグル一覧(カレンダー/
  リマインダー)が空になり、そのまま保存するとデータクラス0個の「停止中」アカウントができてしまう
  現象を発見。7月の実機検証セッションではこの現象に一度も遭遇していなかった(実機には既に
  ユーザー追加アカウントが1つ以上ある状態だったため)。
- **サーバー側の仮説を複数立てて全部潰した**: 「PROPFIND の応答に何か足りないのでは」
  「supported-calendar-component-set の宣言が悪いのでは」「principal 探索の手順に穴があるのでは」
  など、サーバー実装側に原因を求める仮説を順に検証したが、いずれも該当リクエスト/応答に異常は
  見当たらなかった。**決め手になったのは他社サーバーとの対照実験**: 同じまっさら Simulator に
  Vikunja(別実装の CalDAV サーバー)のデモアカウントを追加してみたところ、**同一の症状(トグル
  一覧が空 → データクラス0個の停止中)が再現した**。これにより原因はサーバー実装ではなく iOS
  Simulator 側(ユーザー追加アカウントが1つも無い状態でのアカウント追加フロー固有のバグ)にあると
  確定できた。サーバー側の仮説を先に全部潰してから対照実験で決着させる、という順序を踏んだこと
  自体が今回の調査の核心(「サーバー側で直せるはず」という思い込みだけで時間を溶かさずに済んだ)。
- **回避策の発見**: 「端末を一度 CalDAV 以外のアカウントで温めれば通るのでは」という仮説を検証。
  `xcrun simctl openurl <UDID> 'webcal://<公開icsのURL>'` で照会カレンダー(webcal 購読)を先に
  1つ追加してから CalDAV アカウントを追加すると、初回追加が通ることを確認した(4タップ・テキスト
  入力ゼロで再現可能)。温める効果が CalDAV 固有のものではなく「ユーザー追加アカウントが1つ以上
  存在する」という状態そのものに起因することも確認: 同一端末には「日本の祝日」という購読
  カレンダーが2行(システム管理の `HolidayCalDaemonAccount` とユーザーが webcal で追加したもの)
  並ぶ状態を作り、`sqlite3` で直接データベースを覗いて **システム管理の `HolidayCalDaemonAccount`
  では端末が温まらず、ユーザー追加の照会カレンダーで温まった**ことを確認した(=「アカウントの
  存在」ではなく「ユーザー操作で追加されたアカウントの存在」が条件と判明)。
  `.mobileconfig` によるアカウント投入も回避策の候補として検証したが機能しなかった —
  アカウント系ペイロード全般が Simulator ではアカウントを実際には作らないことが分かり、これは
  CalDAV に固有の制約ではないと結論づけた。
- **今後の検証を高速化するための布石**: 上記の手順で温めた検証用の種デバイス
  `CalDAV-Seed-webcal` を Simulator 上に残置した。以後は `xcrun simctl clone` でこの種デバイスを
  複製するだけで、16.7秒・0タップで CalDAV 接続可能な端末を用意できる(まっさらな端末を毎回
  webcal 温めから作り直す必要がなくなった)。
- **サーバー側の実バグも並行して5件発見・修正(未コミット・未デプロイ)**。iOS の切り分け調査の
  過程で DUMP_DAV_REQUESTS 等を観察しているうちに見つかったもの:
  - 未対応 REPORT への応答が 404(リソース不在)になっていたのを、RFC 3253 §3.6/§1.6 の原文に
    照らして 403 + `<DAV:error><DAV:supported-report/></DAV:error>` に是正。この照合のため
    `docs/rfc/rfc3253.txt` を新規にスナップショット取得してコミット対象に追加した(このリポジトリの
    「RFC の主張は必ず原文で確認する」規律どおり)。
  - `principal-search-property-set` REPORT(RFC 3744 §9.5)が未実装で 404 だったのを実装。
    `principal-property-search` REPORT 自体は未実装なので、あえて空集合を返す設計にした
    (「実装していないものは実装していないと正直に宣言する」という既存の設計原則に沿う判断)。
  - `.well-known/caldav` の 301 リダイレクトに `Cache-Control: no-cache`(RFC 6764 §5 の
    SHOULD)が付いていなかったのを追加。
  - **もっとも重篤だったのが 404 propstat のプロパティ名の破損**: PROPFIND/REPORT で要求された
    プロパティのうちサーバーが持たないものを 404 propstat に列挙する際、要求時の名前空間を
    無視してすべて `DAV:` 名前空間に潰しており、さらにローカル名まで小文字化していた
    (`{http://apple.com/ns/ical/}calendar-color` を要求すると `<d:calendar-color/>` が返る、
    `{urn:ietf:params:xml:ns:caldav}schedule-default-calendar-URL` を要求すると
    `<d:schedule-default-calendar-url/>` が返る、など)。RFC 4918 §14.22 は propstat 内の prop
    要素が「その status に該当するプロパティの名前だけを列挙する MUST」と定めており、名前空間
    URI + ローカル名が一致しなければ別のプロパティとして扱われるため明確な違反。原因は
    200 propstat 側(正しく名前空間を保持していた)とは別のコード経路で 404 propstat 用の名前を
    組み立てていたこと。5つの名前空間すべて・principal/calendar-home/collection すべて・
    PROPFIND と REPORT の両方で再現しており、影響範囲が広かった。**iOS 26.5 は実測でこの壊れた
    応答を許容して動いている**ため、「iOS を壊している証拠がある」わけではなく、「仕様違反である
    ことは確実で、他クライアントや将来の iOS バージョンに対する潜在リスクである」という位置づけで
    記録する(過剰な断定をしない、という docs/rfc/README.md の反省を踏まえた書き方)。
  - `valid-sync-token` の precondition 応答が CalDAV 名前空間で返っていたのを、RFC 6578 §3.2
    原文に照らして `DAV:` 名前空間に修正。
  - `make check` は緑(bun 1105 pass / worker 42 pass)。**push・deploy はしていない**。次回
    deploy 前には、既存クライアントが古い 301(no-cache 無し版)をキャッシュ済みの可能性と、
    principal への PROPFIND で `supported-report-set` が 404 propstat から 200 propstat に
    変わる(=可視の挙動変化がある)ことを踏まえておく。
- **Apple 公式リファレンス実装を使った総当たり調査を実施し、手法として確立した**。
  `/Users/gigun/ghq/github.com/apple/ccs-calendarserver` は Apple 自身が公開している
  CalDAV サーバーの参照実装だが、その中の `simplugin/caldavclient.py`(+ `request-data/`)は
  Apple 自身が「実クライアント(OS X 10.6/10.7/10.11・iOS 5)はサーバーに対してこう振る舞う」と
  モデル化した負荷シミュレータで、リクエストボディが約78本収録されている。このモデルを棚卸しし、
  そのうち約45本を本番サーバーに実際に curl で当てて応答を検証するという手法を今回初めて採用した。
  RFC の条文だけでは見えない「実クライアントが本当に投げるリクエストの形」を体系的に総当たりできる
  ため、**今後の RFC 準拠検証でも再利用できる手法**として位置づける(モデルは2020年で更新停止
  しているため、iOS 26.5 の実機キャプチャ[docs/modeling/06]と食い違う箇所は実機キャプチャの方を
  正とする、という優先順位も今回の調査で確認した)。
  この総当たりで、上記5件の修正では直っていない課題を新たに8件検出した。1件ずつ curl での
  再現コマンドと該当 RFC 条文の原文引用付きで記録し(このセッションの scratchpad
  `apple-client-sweep.md` に詳細を残したが、scratchpad は一時領域のため消える前提で、次セッションが
  着手できる粒度の要約を docs/next-directions.md の「小粒の残タスク」節に書き写した)。8件のうち
  1件(200 propstat 側が要求プロパティの名前空間を見ずに一致判定している問題)は、修正に
  props Record のキー構造そのものを (namespace, localName) 対に変える必要があり、全プロパティ
  定義に波及するため他の7件とは規模が違う別タスクとして切り出した。
- **別件で踏んだ実バグ: `make dev` が起動しない**。custom build の watch プロセスが
  「`*-bundle.ts` を再生成 → その再生成自体をファイル変更として検知 → 再ビルド」という無限
  ループに入ってしまう。本日、このセッションとは別のセッションのエージェントも独立にこの問題を
  踏んでおり、再現性が高いと見られる。ローカル開発体験を直撃するため優先度は高いが、原因はまだ
  調査していない。
