...
- **CalDAV 是正5件はコミット済み・本番確認は一部のみ**: 2026-08-01 の CalDAV 是正5件
  ...
  > **2026-09-06 更新:** 93af616でコミット済みと確認。本番のwell-knownは301 + no-cacheを実測。残り4件の本番確認は未実施。
- **カード配色監査9件(主な表示不具合はローカル修正済み)**: 全件を独立に再検証した監査 →
  ...
  > **2026-09-06 更新:** テーマ適用・CSS変数接続・補助文字/塗り面の配色を修正(未コミット・未デプロイ)。
  > 全チェック・ブラウザ検証PASS(両カードの初期/更新テーマ・変数解除・主要文字/塗り面の比4.5以上)。未使用トークンA-4と既知の実機バグ2件は今回の対象外。
...
- ~~**`make dev` が起動しない**~~ ✅ 同じUI生成物の再書き込みを抑止。起動・HTTP 200・ソース変更後の再起動を実測(未コミット)。
...
1. **2026-08-01 の是正5件を ~~コミット~~ ✅ → 本番反映確認**(未コミットのまま抱えるのが一番危ない)。deploy 前に
...
4. ~~**`make dev` 無限ループの調査・修正**~~ ✅ 2026-09-06 ローカル実測済み。
