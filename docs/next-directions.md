...
**2026-07-14 ドクトリン改訂(ユーザー判断・Fable 同意): 楽観更新へ転換。** 旧「楽観確定しない
(pending スピナー→サーバー確定で反映)」を廃し、**楽観適用 + 失敗時ロールバック + バナー告知**に。
根拠: ①手本の iOS リマインダー自身が楽観更新であり「iOS 準拠」基準と旧方針が矛盾
②実測レイテンシ(数百ms〜秒)で悲観 UI は「クラッシュした?」体験になると実機で確認
③失敗は稀で、稀な失敗のために毎回を遅く見せるのは配分が逆。トースト新設はせず既存エラー
バナー(再試行付き)を失敗面に使う。becoming 語彙は不変(楽観適用の見た目 = becoming を即時に
乗せる)。対象: complete/reopen トグル・quick-add(入力即クリア+行を即挿入)。
> **2026-07-14 追記(ユーザー条件)**: 「1s 以内まで改善できるなら楽観でなくてもよい」。
> Fable 裁定: 速いとき楽観と悲観は見分けがつかず、違うのは遅いときだけ(=楽観が strictly
> dominant)。かつ callServerTool のホストプロキシ区間と tail は制御不能で 1s 保証は不可。
...
- UI: 行の詳細展開(メモ全文・繰り返しバッジ「毎週 月・水」等の人間可読整形・完了時刻)/
  ...
  **UI 分割の設計原則(2026-07-14 Fable 裁定・claude.ai 実測に基づく)**: ホストは
  「1 tool 呼び出し = 1 カード」でカード間遷移 API は無い(会話がルーター)。よって
  ①ページ分割 = ツール×テンプレートの対応(1意図=1ツール=1 ui://)②カード内部の
  展開・折り畳みは自由 ③mega-SPA は避ける。詳細表示は2経路併用: (a) 一覧カード内の
  インライン展開(スライス⑤・追加通信ゼロ)(b) get-todo ツール + 詳細専用テンプレート
  (スライス⑥候補・agent が「詳細見せて」に1カードで応える。編集 UI の置き場もこちら)。
  詳細カードもスナップショットなので app 駆動 refetch を入れること。
  **mega-SPA の境界の哲学(2026-07-14 Fable・ユーザー問いへの裁定)**: 境界は機能数でなく
  「カードが会話からナビゲーション権を奪うか」。意図の内側の操作(完了・追加・詳細展開・
  **コレクション切り替え=同一ビューのパラメータでありナビでない→スライス⑥候補で採用**)は
  カード内 OK。意図が変わる操作(カレンダービュー・横断検索・設定)は新ツール=新カード、
  それ以上は WebUI の管轄。**WebUI との関係は「母艦から切り出す」でなく「共有カーネル
  (contract=Task DTO/TodosViewModel + 純関数群[整形・差分] + コンポーネント)の上に
  意図サイズのカード群と mega が許される面を別々に建てる」**。実行環境が違いすぎるため
  殻の共有は失敗パターン(サンドボックス単一バンドル vs ルーティング/認証/永続状態)。
...
後退する(維持はする)。
**パネル再バインドへの方針(2026-07-14 Fable 裁定)**: 置き換え自体は悪でない —
データ(最新一覧)はホストに従うのが正しく、問題は**ユーザーのビュー選好の黙殺**だけ。
原則: ①パネルモデルはホストの領分・戦わない ②データはホストに従い選好は best-effort で
持ち越す(将来案: sessionStorage に {includeCompleted, calendarId} → 新インスタンス初回に
選好ビューへ取り直し。持続しないホストでは無害 degrade)③「最新カードが生きているカード」の
会話意味論を尊重し古いカードの延命には投資しない。段階論: 今は何もしない(claude.ai は
カード積みで再バインド無し)→ 実運用で不快が実証されたら sessionStorage 復元 →
マルチホスト時に principal 既定ビューのサーバー昇格。
