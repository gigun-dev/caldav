...
| B8 | サーバー側削除は sync-report の 404(RFC 6578 — 前作の 410 は誤りと原文照合済み)で iOS に伝わる | 05 訂正2 | サーバー側でリソースを消して iOS の同期を観測 | ⬜ 本キャプチャはサーバー側削除の操作を含まないため未測(本作スケルトンで実験系として実施) |
