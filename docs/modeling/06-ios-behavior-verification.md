...
### 【重要】上記1〜4とは性質が異なる第5の条件 — サーバー側では直せない(2026-08-01 実測)

上記1〜4は前作/開発初期サーバーの実装不備であり、**サーバーを直せば消える**条件だった。
以下はまっさらな iOS Simulator に限って発生する別の現象で、**サーバーを直しても消えない**。
次にこの文書を読む人が「またサーバーのバグだ」と早合点しないよう、意図的に節を分けて記録する。

#### 症状

ユーザー追加アカウントが **1つも無い** iOS Simulator(新規作成直後)で、設定 → アプリ →
カレンダー → アカウント → アカウントを追加 → その他 → CalDAV アカウント を実行すると、
**サーバー検証は成功する**(iOS 側ログに `Authenticator determined credentials are valid` が
出る)のに、次画面の「カレンダー/リマインダー」トグル一覧が **空** になる。そのまま保存すると
データクラス0個 = 「停止中」のアカウントができる。

#### 回避策(実測)

1回目を **保存まで進めて** からそのアカウントを削除し、もう一度追加すると成功する
(**キャンセルでは駄目** — 保存という操作自体が要る)。一度どれか1つ CalDAV アカウントが
成立した端末では、**別ホストの2つ目以降は1回目から成功する**。

#### 対照実験(すべて新規作成した Simulator で実測)

| 条件 | 1回目でトグルが出るか |
|---|---|
| 新品 + 本作サーバー(`caldav.gigun-dev.workers.dev`) | ❌ |
| 新品 + 同上(事前にリマインダー/カレンダー App を起動して温めた) | ❌ |
| 新品 + Cloud Run プロキシ(`caldav-proxy-...run.app`。ポート 8443 に応答しない入口) | ❌ |
| 新品 + **無関係な他社 CalDAV サーバー**(Vikunja のデモ `try.vikunja.io`) | ❌ |
| 既に CalDAV アカウントが1つある端末 + 別ホスト | ✅ |

→ **無関係な他社サーバー(Vikunja)でも再現するので、本作の実装は原因ではない。** これが
「サーバー側で直せる条件」と本節を区別する決め手。

#### 根拠となる観測(実測)

- **失敗する1回目と成功する2回目(同一ホストへの delete→再追加)がサーバーへ送るリクエストは、
  6件でバイト単位に同一**(`.well-known/caldav` → `/dav/` → `OPTIONS {principal}` を 443/8443 の
  2経路ぶん)。**2回目はサーバーに追加の問い合わせを一切していない** — 判断材料は端末内の
  キャッシュ由来だと分かる。
- 両試行ともサーバーの応答は同一(301 / 207 / 204)。accountsd のログに
  `Authenticator determined credentials are valid` が出る。**検証自体は通っている。**
- **両試行とも `PROPFIND {principal}` にも `PROPFIND {calendar-home}` にも到達していない。**
  → 下記「成功時の探索・初回同期シーケンス」に記録した7ステップのうち4手目
  (`PROPFIND {principal}`)以降が起きていない。つまり**あの節の「トグル一覧が出る」は
  「コレクション列挙に成功した印」ではなく「サーバー検証 OK の印」だった**、という訂正が要る
  (あの節自体の観測 — 既存アカウントがある実機での7ステップ — は誤りではないが、
  「トグルが出る = コレクションを見つけた」という読みが誤りだったと今回分かった)。
- 検証成功の **1.00秒後**(5回中4回で 1.003〜1.009秒)に ACUI(アカウント設定 UI)が
  **自動保存** を走らせ、トグル画面は **その保存の後** に出る。「保存すると停止中のアカウントに
  なる」は独立した障害ではなく、トグルを1つも ON にせずこの画面を抜けた結果として自然に起きる。
- 成功時にだけ `com.apple.accounts.dom`(accountsd のデータクラス所有者ヘルパ)が remindd を
  起動している(`AAACCOUNTS` を含む LAUNCH EVENT)。観測5件で 5/5 一致。

#### なぜ7月の検証で気づかなかったか

7月の検証(本文書「検証環境」節)は **実機** で行っていた。実機には Apple ID やメールアカウントが
既に存在するため「ユーザー追加アカウントがゼロ」という状態にならず、この現象には当たらない。
**まっさらな Simulator に特有の、初回限定の現象。**

#### RFC との関係(実測と符合。原文 `docs/rfc/rfc6764.txt` §6 で確認)

> Once a successful account discovery step has been done, clients SHOULD cache the service
> details that were successfully used (user identity, principal-URL with full scheme/host/port
> details) and reuse those when connecting again at a later time.
> (`docs/rfc/rfc6764.txt` §6、L403-406)

「保存しないと2回目が通らない」という実測はこの規定と符合する(保存 = successfully used、
という解釈)。ただしこの一文は「成功後の再利用」を規定しているだけで、**「初回にトグルが
空になる」現象そのものの原因を説明するものではない**。RFC を満たすための iOS 内部実装
(データクラス所有権の初期化と自動保存のタイミング)は非公開であり、これ以上は推測の域を出ない。

#### 推論(未検証。実測と混同しないこと)

観測(`com.apple.accounts.dom` の remindd 起動タイミング、検証成功から自動保存までの
約1秒という短い猶予)から、「データクラス所有者ヘルパのコールドスタートが ACUI の自動保存
(約1秒後)に間に合わないレース条件」という説明が立てられる。**ただしこれは推論であり、
実測で確証したものではない。** サーバー応答をどう変えても関与しない層(そもそも
`PROPFIND {principal}` に到達していない)で起きているため、これ以上の要因分解には iOS
内部の計測が必要で、本調査の手段(Proxyman 復号キャプチャ)の範囲外。

#### 検証中に潰した仮説(短く記録する価値がある)

`:8843` ポートプローブのタイムアウト / MITM プロキシの証明書 / DAV compliance トークンの
不足 / 443+8443 の二重チェーン / リマインダー App の温め — **すべて実測で否定**。
「サーバー側で打てる手」を探して全部外した、という経緯自体が次の人の時間を守る。

#### 結論: サーバー側では対処不可

無関係な他社サーバー(Vikunja)でも再現し、かつ要求プロパティ・href 正規化・Content-Length
など上記1〜4でサーバー応答を変えて解決してきたレイヤに一切到達していない
(`PROPFIND {principal}` すら送られていない)。**この症状に対してサーバー実装でできることは
無い。** iOS 実機(既にアカウントがある環境)で検証するか、Simulator では「ダミーアカウントを
1回保存してから本番アカウントを追加する」という運用上の回避策で迂回するしかない。

### 同じ調査中に検出した独立のサーバー側課題(検出済み・対応中。上記の第5条件とは無関係)

上記の Simulator 限定現象そのものとは無関係だが、同じキャプチャ調査で見つかった実バグ
2件(別エージェントが修正中)。

- **`REPORT principal-search-property-set` に 404 を返している。** RFC 3744 §9.5
  (`docs/rfc/rfc3744.txt` L3128)は "Servers MUST support the DAV:principal-search-property-set
  REPORT on all collections identified in the value of a DAV:principal-collection-set property"
  と規定しており、404 は仕様違反。Apple のリファレンス実装(`ccs-calendarserver` の
  `simplugin/caldavclient.py`)のブートストラップがこの REPORT を投げる経路で観測。
- **`.well-known/caldav` の 301 に `Cache-Control` が無い。** RFC 6764 §5
  (`docs/rfc/rfc6764.txt` L218)の "Servers SHOULD set an appropriate Cache-Control header
  value ... in the redirect response" に対する未対応。
...
> 2026-08-01 追記: 上記7ステップは「既に CalDAV アカウントがある実機」での観測であり、
> それ自体は正しい。ただし **まっさら Simulator の初回試行では 4 手目(`PROPFIND {principal}`)
> に到達する前に UI 側が自動保存へ進んでしまう**別の現象があり(サーバー実装非依存)、
> 「アカウント追加成功=このシーケンスを最後まで踏む」と早合点しないこと。詳細は上の
> 「【重要】上記1〜4とは性質が異なる第5の条件 — サーバー側では直せない」節を参照。
