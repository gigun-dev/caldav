...
<style>
  ...
  * { -webkit-tap-highlight-color: transparent; }
  ...
  /* 【2026-07-18 死コード化(ユーザー裁定「完了も追加もテキスト不要」)】entry.ts の renderRow は
   * もう .tag 要素(tagEl)を生成しない — becoming の視覚は丸チェックの塗り/リング等の非テキスト
   * 演出だけに一本化した(aria-live の音声通知は announceBecoming に残る)。以下 .tag 系セレクタ
   * (このブロック〜 li.becoming-*.tag / .meta .tag / .row-main > .tag 系)は DOM に要素が無いため
   * 常に無効だが、becoming の form(リング等)の実装史・レイアウト調整史(v2.1 修正C 等)の記録
   * として削除せず残す(コメント規律: 消すのは事実として誤りのときだけ)。
  ...
  /* C0-a(撤去済み・2026-07-23): 完了行を「タイトルだけグレーフェード → 約3秒で畳んで退場」させていた
