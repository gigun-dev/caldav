...
export interface TodosViewModel {
	...
	uiHash?: string;
	/**
	 * 【2026-07-24 実機フィードバック #48/#50・是正C】create-todo/create-todos が新規作成した
	 * タスクの UID。「今回のモデル操作で見せるべき1件」をカードへ伝える唯一のチャンネル。
	 *
	 * 【なぜ必要か(新規タスクが「他 n件」の裏に埋もれる実害)】inline カードは INLINE_PREVIEW_MAX
	 * (fold.ts)件でクランプし、alphabetical(タイトル順)に並べる。モデルが create-todo で作った
	 * 新規タスクのタイトルが「た行」「わ行」寄りだと、そのままでは可視 5 件に入らず「他 n件」の裏に
	 * 埋もれて、ユーザーが「追加したのに一覧に出てこない」と誤解する(#48/#50 の実機報告)。
	 * 【なぜ setTimeout フラッシュではなく highlightId(状態駆動)にしたか】このリポジトリのカード
	 * ドクトリン「時間駆動の視覚イベントを型から排する」(CLAUDE.md・このタスクの指示書参照)により、
	 * フェード/自動消滅ハイライトのような時限演出は選択肢から外れる。highlightId は「サーバーが
	 * 今回の応答で surface すべき1件を名指しする」だけの状態値で、カード(todos-entry.ts)側は
	 * これを「常にクランプを突破して可視セットへ強制編入する」「一度だけ scrollIntoView する」という
	 * 決定論的な描画規則の入力として使うだけ(時間経過による見た目の変化は一切発生しない)。
	 * 【なぜ単一(配列でない)か】create-todos(複数件)でも「今スクロールして見せる代表1件」があれば
	 * 目的(埋もれ防止)を達成できる——全件を同時に強制可視化すると N 件バッチのときに inline の
	 * 有界性そのものが壊れる(5件クランプの意味が無くなる)。create-todos ハンドラは成功した最後の
	 * 1件(succeeded 配列の末尾 = 追加順で最後に作られた行)を代表として載せる。
	 * 【additive・後方互換】refresh-todos/list-todos や複数の mutate(update/complete/delete/move)
	 * など「新規作成を伴わない」応答はこのフィールドを載せない(undefined)。旧 UI/旧テスト
	 * フィクスチャは不在を前提にしているため optional。
	 */
	highlightId?: string;
}
