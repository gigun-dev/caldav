...
- **RFC の主張を確認するときは必ず `docs/rfc/` の原文(全文スナップショット)を読むこと。**
  ...
  照合結果は docs/modeling/05 に記録。詳細は `.claude/rules/rfc-primary-sources.md`
  (domain/docs 編集時に自動ロード)。
...
## 情報の書き分け方針(重要・このリポジトリの基本ルール)

**媒体ごとに担う情報を分ける**(重複させず、情報を迷子にしない):

- **コード = How** — どう実現しているか。
- **テストコード = What** — 何を保証するか(仕様・振る舞い)。テスト名/構造で表現する。
- **コミットログ = Why** — なぜその変更をしたか・経緯。
- **コードコメント = Why not** — なぜ他の案を採らなかったか・ボツ案・落とし穴。

**コメントはコードと同量レベルでベッタベタに書く。** コメントの詳細は
