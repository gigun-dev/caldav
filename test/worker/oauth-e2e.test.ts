...
describe("OAuth-for-MCP E2E(スライス2)", () => {
	...
	it("(d) /mcp: OAuth access_token で tools/list を呼ぶと5ツールが見える(E-1 create-todo/list-todos 追加)", async () => {
		...
		expect(toolNames).toEqual(["complete-todo", "create-calendar", "create-event", "create-events", "create-todo", "create-todos", "delete-calendar", "delete-event", "delete-todo", "diag-card", "get-current-time", "get-freebusy", "list-calendars", "list-deleted", "list-events-expanded", "list-known-locations", "list-todos", "move-todo", "refresh-events", "refresh-todos", "report-card-telemetry", "restore-deleted", "search-location", "update-calendar", "update-event", "update-todo"]);
	});
