...
describe("OAuth-for-MCP E2E(スライス2)", () => {
	...
	it("(d) /mcp: OAuth access_token で tools/list を呼ぶと5ツールが見える(E-1 create-todo/list-todos 追加)", async () => {
		...
		expect(toolNames).toEqual(["complete-todo", "create-calendar", "create-todo", "delete-todo", "get-current-time", "get-freebusy", "list-calendars", "list-events-expanded", "list-todos", "refresh-todos", "update-todo"]);
	});
